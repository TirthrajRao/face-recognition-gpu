...
counter = 0#This will help us to skip the no. of frame .
correctPredictedPerson = False
correctlyPredictedFace = []
...
def recognisePerson(label):
    var = input("Are you a : "+ label+ " --> Enter( y / n ) - ")
    print("i entered ========> ", var)
    if(var == 'y'):
        return True
    else:
...
def createNewDataSet(frame):
    resultName = input("Enter Face Label -->   ")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        while True:
            ...
            print("Correctly predicted face ========> ", correctlyPredictedFace, "counter value ======+> ", counter)
            # print(" ========> ", best_class_probabilities[0] * 100 < 4, best_class_probabilities[0] * 100 )
            if(counter == 20):
                counter = 0
                if(resultName is not '' ) and (resultName not in correctlyPredictedFace ):
                    if(best_class_probabilities[0] * 100 < 4):
                        cv2.waitKey(1000)
                        correctPredictedPerson = recognisePerson(resultName)
                        print("correctPredictedPerson ===============> ", correctPredictedPerson)
                        if(correctPredictedPerson == True):
                            correctlyPredictedFace.append(resultName)
                        else:
                            var = input("Do you want to save this image as new face? ( y / n ) ")
                            if(var == 'y'):
                                createNewDataSet(frame)
                                correctlyPredictedFace.append(resultName)
                                correctPredictedPerson = True
                    else:
                        correctlyPredictedFace.append(resultName)
